...
assert.match(render(), /Báo cáo chuyên đề/);
assert.doesNotMatch(render(), /Môi trường: Báo cáo chuyên đề/);
const source=fs.readFileSync(appPath,'utf8');
assert.match(source, /scheduleEnvironmentSelectClass/);
assert.match(source, /aria-label="Môi trường dạy"/);
assert.ok(!source.includes('Môi trường: {option.label}'));
