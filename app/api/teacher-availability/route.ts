...
export async function POST(request: Request) {
  ...
  try {
    ...
    const isWithdraw = rawScope === "none";
    if (!isWithdraw && !availabilityScopes.includes(rawScope as TeacherAvailabilityScope)) {
      return apiFailure(400, "Lựa chọn thời gian đăng ký không hợp lệ.", undefined, requestId);
    }
    const scope = rawScope as TeacherAvailabilityScope;
    const timeSlotIds = parseIds(body.timeSlotIds);
    if (scope === "time_slots" && timeSlotIds.length === 0) {
    ...
    const activeSlotIds = new Set(
      slots
        .filter((slot) => String(slot.active || "true").toLowerCase() !== "false")
        .map((slot) => String(slot.id || "").trim()),
    );
    if (scope === "time_slots" && timeSlotIds.some((id) => !activeSlotIds.has(id))) {
    ...
    const rowsToCreate: TeacherAvailability[] = isWithdraw
      ? []
      : dates.flatMap((date) => {
          const selectedSlots = scope === "time_slots" ? timeSlotIds : [""];
    ...
    await appendAuditLog({
      ...
      after: { dates, scope: isWithdraw ? "none" : scope, timeSlotIds, createdRows: rowsToCreate.length },
    });
